...
class InstagramGame(Game):
    def __init__(self, game_master: bool = None):
        ...
        self.get_new_messages = get_new_messages
        self.receive = True
        self.send_messages = []
        Timer(1, self.start_recieve_message).start()
        ...
        if self.config["register_type"] == "liste":
            ...
            with open("players.json", "r", encoding='utf-8') as f:
                ...
                self.players = [InstaPlayer(player["name"], player["username"], used_passwords, used_id)
                                ...
                                player.get("play", True) and player.get("type", "") == "instagram"]
            ...
            url_label = Label(qrcode_frame,
                              text=f"Envoyez {self.register_code} à {self.config['insta_username']} pour vous enregistrer",
                              font=("Arial", 28))
            url_label.pack()
    ...
    def send_info_all(self, message: str):
        ...
        for player in self.players:
            ...
            self.send_message(player.username, new_message)

    def send_info(self, player: InstaPlayer, message: str):
        print(player.name, ":", message)
        self.send_message(player.username, message)
    ...
    def check_command(self, player, content: str) -> bool:
        """
        ...
        for task in player.tasks:
            if task.type == "validate_basic":
                ...
                for word in task.activ_keywords:
                    if word in content:
                        self.send_message(player.username, f"La tâche {task.name} vous envoie:\n{task.message}")
                        task.active = True
                        return True
                ...
                for word in task.keywords:
                    if word in content:
                        if task.active:
                            self.task_done(player, task)
                            self.send_message(player.username, "Vous avez bien validé la tâche !")
                            return True
                        else:
                            self.send_message(player.username, "La tâche n'est pas encore activée")
                            return True
                for word in task.activ_keywords:
                    if word in content:
                        self.send_message(player.username, f"La tâche {task.name} vous envoie:\n{task.message}")
    ...
    def start_recieve_message(self):
                    ...
                    for msg in new:
                        ...
                        if self.import_window is not None:
                            if self.register_code in msg["text"]:
                                ...
                                player = InstaPlayer(new_player["name"], new_player["username"], self.used_passwords,
                                                     self.used_id)
                                ...
                                self.import_players()
                                continue
                        elif self.check_command(joueur, msg["text"]):
                            continue
                        else:
        ...
        print("Démarrage de la vérification des messages")
